...
export default function NewShop() {
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      try {
        const res = await axios.get(`${API_URL}/allshop`);
        if (!mounted) return;
  ...
  const handleEdit = (id) => navigate(`/admin/newshop/edit/${id}`);

  const handleDelete = async (id) => {
    try {
      setDeletingId(id);
      await axios.delete(`${API_URL}/shopdelete/${id}`);
    ...
    <div className="content-wrapper container my-5 d-flex justify-content-center align-items-center">
      ...
      >
        <div className="card-header d-flex align-items-center justify-content-between">
          <h1 className="mb-0 all-employee">All Shops</h1>
          ...
          <div className="d-flex flex-row gap-2">
            <button
              ...
              onClick={() => navigate("/admin/newshop")}
            >
              + Add Shop
            </button>
          </div>
            ...
            <>
                    ...
                    {Array.isArray(pagedEmployees) &&
                      pagedEmployees.map((emp, idx) => (
                        <tr key={emp.id ?? startIndex + idx}>
                          ...
                          <td>
                            {emp.description
                              ? emp.description.substring(0, 10) +
                                (emp.description.length > 10 ? "..." : "")
                              : "-"}
                          </td>
                          ...
                          <td className="text-center">
                            <button
                              type="button"
                              className="btn btn-sm btn-outline-primary me-2 create-crud mb-1"
                            ...
                            <button
                              type="button"
                              className="btn btn-sm btn-outline-danger create-crud mb-1"
              ...
              {totalEmployees > itemsPerPage && (
                <div className="d-flex justify-content-center mb-3 align-items-center mt-3">
                  <nav aria-label="Shops table pagination">
        ...
        <div className="card-footer text-muted small">
          {employees.length} shop{employees.length !== 1 ? "s" : ""}
        </div>
      </div>
    </div>
  );
